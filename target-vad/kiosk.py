...
def _build_runtime(config: dict) -> _LazyDirectorRuntime:
    ...
    grammar = None
    no_markdown_grammar = llm_cfg.get("no_markdown_grammar", False)
    if no_markdown_grammar is not True:
        if "no_markdown_grammar" in llm_cfg and not isinstance(no_markdown_grammar, bool):
            print(f"[kiosk] llm.no_markdown_grammar is not a boolean "
                  f"(got {no_markdown_grammar!r}) -> treating as disabled",
                  file=sys.stderr, flush=True)
    else:
        from modes.talkback.gbnf import SPEECH_GRAMMAR
        grammar = SPEECH_GRAMMAR
    cache_prompt = llm_cfg.get("cache_prompt", False)
    if cache_prompt is not True and "cache_prompt" in llm_cfg and not isinstance(cache_prompt, bool):
        print(f"[kiosk] llm.cache_prompt is not a boolean "
              f"(got {cache_prompt!r}) -> treating as disabled",
              file=sys.stderr, flush=True)
    llm = LlmClient(
        ...
        grammar=grammar,
        cache_prompt=cache_prompt is True,
    )
